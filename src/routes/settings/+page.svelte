<script lang="ts">
	...
	function pressPin(key: string) {
		...
		if (key === 'del') {
			if (pinStep === 'set') newPin = newPin.slice(0, -1);
			else confirmPin = confirmPin.slice(0, -1);
			return;
		}
		if (key === 'clr') {
			if (pinStep === 'set') newPin = '';
			else confirmPin = '';
			return;
		}
		if (cur.length < pinLength) {
			if (pinStep === 'set') newPin += key;
			else confirmPin += key;
		}
	}
	...
	async function saveAppSettings() {
		...
		try {
			...
			if (res.ok) {
				appSaved = true;
				setTimeout(() => (appSaved = false), 2500);
			} else {
				const d = await res.json();
				appError = d.error ?? 'Failed to save.';
			}
		} catch {
	...
	async function doSavePin() {
		...
		try {
			...
			if (res.ok) {
				...
				confirmPin = '';
				pinStep = 'set';
				setTimeout(() => (pinSaved = false), 2500);
			} else {
				const d = await res.json();
	...
	async function addTracker() {
		if (!tName.trim()) {
			trackerError = 'Name is required.';
			return;
		}
		if (tOnWeeks < 1 || tOffWeeks < 1) {
			trackerError = 'Week counts must be at least 1.';
			return;
		}
		trackerError = '';
		savingTracker = true;
		try {
			...
			if (res.ok) {
				...
				const d = await res.json();
				trackerError = d.error ?? 'Failed to create tracker.';
			}
		} catch {
			trackerError = 'Network error.';
		} finally {
			savingTracker = false;
		}
	}
	...
	// ── Notifications / PWA ──────────────────────────────────────────────────────
	let notifStatus = $state<'unknown' | 'unsupported' | 'denied' | 'granted' | 'subscribed'>(
		'unknown'
	);
...
<div class="min-h-screen bg-linear-to-br from-slate-950 via-violet-950 to-fuchsia-950">
	...
	<main class="mx-auto max-w-lg space-y-6 px-4 py-6">
		...
		<section class="rounded-2xl border border-white/10 bg-white/5 p-5">
			<h2 class="mb-4 text-sm font-bold tracking-wider text-white/50 uppercase">App Settings</h2>
			<div class="space-y-4">
				<div>
					<label class="mb-1.5 block text-xs font-semibold text-white/70" for="appNameInput"
						>App name</label
					>
					<input
						id="appNameInput"
				...
				<div>
					<label class="mb-1.5 block text-xs font-semibold text-white/70" for="sessHours"
				...
				<label
					class="flex cursor-pointer items-center justify-between rounded-xl border border-white/10 bg-white/10 px-4 py-3"
				>
					<span class="text-sm text-white/80">Lock when browser closes</span>
					<input
						type="checkbox"
						bind:checked={lockOnClose}
						class="h-4 w-4 rounded accent-fuchsia-500"
					/>
				</label>
			</div>
		...
		<!-- PIN settings -->
		<section class="rounded-2xl border border-white/10 bg-white/5 p-5">
			<h2 class="mb-4 text-sm font-bold tracking-wider text-white/50 uppercase">Security</h2>
			{#if pinSaved}
				<p class="mb-3 rounded-xl bg-emerald-500/10 px-4 py-2 text-sm text-emerald-400">
			...
			{#if !showPinChange}
				<button
					...
					class="w-full rounded-xl border border-white/10 bg-white/5 py-2.5 text-sm font-semibold text-white/70 transition hover:bg-white/10"
				>
					Change PIN
				</button>
			{:else}
				<div class="rounded-xl border border-white/10 bg-white/5 p-4">
					<p class="mb-3 text-center text-sm font-semibold text-white">
						{pinStep === 'set' ? 'Enter new PIN' : 'Confirm new PIN'}
					</p>
						...
						<div class="mb-4 flex items-center justify-center gap-2">
							...
							{#each [4, 5, 6] as n (n)}
								<button
									...
									class="rounded-lg px-3 py-1 text-xs font-semibold {pinLength === n
										? 'bg-fuchsia-500 text-white'
										: 'bg-white/10 text-white/50 hover:bg-white/20'}">{n}</button
								>
							{/each}
						</div>
					{/if}
					...
					<div class="mb-4 flex justify-center gap-3">
						{#each Array.from({ length: pinLength }, (_, i) => i) as i (i)}
							<div
								class="h-4 w-4 rounded-full border-2 transition-all {curPinDisplay.length > i
					...
					<div class="grid grid-cols-3 place-items-center gap-2">
						{#each PAD as key (key)}
							{#if key === 'clr'}
								<button
									onclick={() => pressPin('clr')}
									aria-label="Clear"
									class="flex h-12 w-12 items-center justify-center rounded-full text-purple-300 active:scale-95"
								>
									<svg
										xmlns="http://www.w3.org/2000/svg"
										class="h-4 w-4"
							...
							{:else}
								<button
									...
									class="h-12 w-12 rounded-full bg-white/10 text-base font-semibold text-white hover:bg-white/20 active:scale-95"
									>{key}</button
								>
							{/if}
						{/each}
					...
					<button
						...
						class="mt-3 w-full text-center text-xs text-white/40 hover:text-white/60">Cancel</button
					>
				</div>
			{/if}
		</section>
		...
		<section id="add-tracker" class="rounded-2xl border border-white/10 bg-white/5 p-5">
			<div class="mb-4 flex items-center justify-between">
				<h2 class="text-sm font-bold tracking-wider text-white/50 uppercase">Trackers</h2>
				<button
					onclick={() => {
						showAddTracker = !showAddTracker;
						trackerError = '';
					}}
					class="rounded-lg bg-fuchsia-500/20 px-3 py-1 text-xs font-bold text-fuchsia-400 transition hover:bg-fuchsia-500/30"
				>
			...
			<div class="mb-4 space-y-2">
				{#each trackers as t (t.id)}
					<div class="flex items-center gap-3 rounded-xl bg-white/5 px-4 py-3">
						...
						<a
							href={resolve('/tracker/[id]', { id: String(t.id) })}
							class="shrink-0 text-xs text-fuchsia-400 hover:underline">View</a
						>
						<button
							onclick={() => deleteTracker(t.id)}
							class="shrink-0 text-white/30 transition hover:text-red-400"
			...
			<!-- Add tracker form -->
			{#if showAddTracker}
				<div class="space-y-3 rounded-xl border border-fuchsia-500/30 bg-fuchsia-500/5 p-4">
					...
					<div>
						...
						<input
							...
							bind:value={tName}
							maxlength="100"
							class="w-full rounded-xl border border-white/10 bg-white/10 px-4 py-2 text-sm text-white placeholder-white/30 focus:border-fuchsia-400 focus:outline-none"
					...
					<div class="grid grid-cols-2 gap-3">
						<div>
							...
							<input
								...
								min="1"
								max="52"
								class="w-full rounded-xl border border-white/10 bg-white/10 px-4 py-2 text-sm text-white focus:border-fuchsia-400 focus:outline-none"
							/>
						</div>
						<div>
					...
					<div class="grid grid-cols-2 gap-3">
						<div>
							...
							<input
								id="newTDate"
								type="date"
								bind:value={tStartDate}
								class="w-full rounded-xl border border-white/10 bg-white/10 px-4 py-2 text-sm text-white scheme-dark focus:border-fuchsia-400 focus:outline-none"
							/>
						</div>
						<div>
							...
							<div class="flex gap-2 pt-0.5">
								<button
									onclick={() => (tStartPhase = 'ON')}
									class="flex-1 rounded-xl py-2 text-xs font-bold {tStartPhase === 'ON'
										? 'bg-emerald-500 text-white'
										: 'bg-white/10 text-white/50'}">ON</button
								>
								<button
									onclick={() => (tStartPhase = 'OFF')}
									class="flex-1 rounded-xl py-2 text-xs font-bold {tStartPhase === 'OFF'
										? 'bg-slate-500 text-white'
										: 'bg-white/10 text-white/50'}">OFF</button
								>
							</div>
						</div>
					</div>
						...
						<div class="flex flex-wrap gap-2">
							{#each COLORS as c (c)}
								<button
									...
									style="background:{c}"
									aria-label={c}
									class="h-6 w-6 rounded-full {tColor === c
					...
					<button
						...
						class="w-full rounded-xl bg-fuchsia-500 py-2.5 text-sm font-bold text-white transition hover:bg-fuchsia-600 disabled:opacity-50"
					>
		...
		<section class="rounded-2xl border border-white/10 bg-white/5 p-5">
			...
			{:else if notifStatus === 'denied'}
				<p class="text-sm text-yellow-400/80">
					Notifications are blocked. Open your browser/OS settings to re-enable them, then come back
					here.
				</p>
			...
			{:else}
				<p class="mb-3 text-sm text-white/60">
					Get a daily reminder if you haven't checked in yet.
				</p>
				{#if !isStandalone}
		...
		<!-- Logout -->
		<section class="rounded-2xl border border-white/10 bg-white/5 p-5">
			<h2 class="mb-4 text-sm font-bold tracking-wider text-white/50 uppercase">Session</h2>
			<button
				onclick={logout}
				class="w-full rounded-xl border border-red-500/30 bg-red-500/10 py-2.5 text-sm font-semibold text-red-400 transition hover:bg-red-500/20"
			>
				Lock app / Log out
			</button>
		</section>
