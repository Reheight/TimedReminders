...
export const PUT: RequestHandler = async ({ request, locals }) => {
	...
	if (b.appName !== undefined) {
		...
		await setConfig(CONFIG_KEYS.APP_NAME, b.appName.trim().slice(0, 50), {
	...
	if (b.sessionHours !== undefined) {
		...
		if (!Number.isInteger(h) || h < 1 || h > 8760)
			return json({ error: 'Invalid session duration' }, { status: 422 });
		await setConfig(CONFIG_KEYS.SESSION_DURATION_HOURS, String(h), {
	...
	if (b.lockOnClose !== undefined) {
		await setConfig(CONFIG_KEYS.LOCK_ON_CLOSE, b.lockOnClose ? 'true' : 'false', {
			displayName: 'Lock on Browser Close',
			valueType: 'BOOLEAN',
			scope: 'SECURITY'
		});
	}
	...
	if (b.vapidPublicKey !== undefined) {
		if (typeof b.vapidPublicKey !== 'string')
			return json({ error: 'Invalid VAPID public key' }, { status: 422 });
		await setConfig(CONFIG_KEYS.VAPID_PUBLIC_KEY, b.vapidPublicKey.trim(), {
	...
	if (b.vapidPrivateKey !== undefined && b.vapidPrivateKey !== '') {
		if (typeof b.vapidPrivateKey !== 'string')
			return json({ error: 'Invalid VAPID private key' }, { status: 422 });
		await setConfig(CONFIG_KEYS.VAPID_PRIVATE_KEY, b.vapidPrivateKey.trim(), {
	...
	if (b.vapidSubject !== undefined) {
		if (typeof b.vapidSubject !== 'string')
			return json({ error: 'Invalid VAPID subject' }, { status: 422 });
		await setConfig(CONFIG_KEYS.VAPID_SUBJECT, b.vapidSubject.trim(), {
	...
	if (b.cronSecret !== undefined && b.cronSecret !== '') {
		if (typeof b.cronSecret !== 'string')
			return json({ error: 'Invalid cron secret' }, { status: 422 });
		await setConfig(CONFIG_KEYS.CRON_SECRET, b.cronSecret.trim(), {
